 ...
 *                                                                            *
 * Downloaded from https://github.com/raccoon-dev/JsonReader                  *
 *                                                                            *
 ******************************************************************************}
...
{ TJsonReader }
 ...
 TJsonReader = class(TObject)
  ...
  public
    ...
    function IsPropertyName(const AName: string = ''): Boolean;
...
function TJsonReader.IsPropertyName(const AName: string = ''): Boolean;
begin
  Result := (TokenType = {$IFDEF FPC}TJsonToken.jtPropertyName{$ELSE}TJsonToken.PropertyName{$ENDIF});
  if Result and (AName <> '') then
    Result := SameText(PropertyName, AName);
end;
...
begin
  ...
  begin
    if (TokenType = {$IFDEF FPC}TJsonToken.jtString{$ELSE}TJsonToken.String{$ENDIF}) then
      Result := ValueAsString else
    if (TokenType = {$IFDEF FPC}TJsonToken.jtNull{$ELSE}TJsonToken.Null{$ENDIF}) then
      Result := ''
    else
      raise Exception.Create('String value expected for property ' + PropertyName);
  end else
